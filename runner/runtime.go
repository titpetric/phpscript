...
// produced it.
//
// While output buffering is active (ob_start) this is the innermost buffer, so
// everything the script emits is captured rather than sent on.
func (rt *Runtime) Output() io.Writer {
	if n := len(rt.obStack); n > 0 {
		return rt.obStack[n-1]
	}
	return rt.out
}
