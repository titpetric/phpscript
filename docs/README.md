...
Variable types are dynamic, as in PHP. One-type-per-name was enforced at
run time for a short while and reversed: existing code retypes variables
freely and the cost of fixing it outweighed the guarantee. The convention
survives as an advisory `phpscript lint` finding ("no reassignment: $a
previously declared as string") that flags a literal reassigning a variable
at another type without failing the run. The runtime behaves exactly as PHP
does; `tests/fixtures/types/` pins that on both engines with php as the
oracle.
