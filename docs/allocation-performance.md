...
`$obj->field`, `$obj->method()` all work against native Go values through
...
Legend: **OK** — optimal, nothing to do. **OK (by design)** — allocates, but
the allocation buys required semantics. **TODO** — a real improvement is
available.
