...
Plain assignment to a variable can appear inside larger expressions, including conditions. The linter reports assignment inside `if` conditions because it is easy to mistake for comparison. Array, property, destructuring, and compound assignment expressions are supported only as standalone statements.
...
`include`, `include_once`, `require`, and `require_once` are recognized. The `*_once` keywords execute a file only when it has not run yet, and the record they consult covers every include: a plain `include` earlier in the request already satisfies a later `include_once`.
