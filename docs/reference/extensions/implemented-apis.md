...
This file lists the functions and classes the standard phpscript CLI runtime registers, grouped by the Go package each binding lives in. The comment over a binding's registration site (or the godoc of the function it registers) is the line shown here, and the signature is derived from the Go implementation. Regenerate with `atkins test:introspection` after changing a registration or its comment; docs/naming-conventions.md documents the comment convention.
