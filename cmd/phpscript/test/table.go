...
func (t *markdownTable) writeSummary(passed, failed, total int, duration time.Duration) {
	writeMarkdownSummary(t.w, t.totals, passed, failed, total, duration, t.lat || t.loop || t.profile, nil)
}
...
// A matrix run passes its engines and gets a duration column per runner
// beside the wall-clock one.
func writeMarkdownSummary(w io.Writer, totals []groupTotals, passed, failed, total int, duration time.Duration, timings bool, engines []engineDuration) {
	...
	if timings {
		...
		for _, engine := range engines {
			header := matrixHeaders[engine.Runner] + " (ms)"
	...
	for _, group := range totals {
		...
		if timings {
			...
			for _, engine := range engines {
				values = append(values, formatDuration(engineTime(group.Engines, engine.Runner)))
			}
		}
		row(values)
	}
	...
	if timings {
		...
		for _, engine := range engines {
			values = append(values, formatDuration(engine.Duration))
		}
	}
	row(values)
	fmt.Fprintln(w)
...
// engineTime picks one runner's share out of a totals row.
func engineTime(engines []engineDuration, runner tests.Runner) time.Duration {
	for _, engine := range engines {
		if engine.Runner == runner {
...
// formatEngineSplit renders the duration a summary line closes with. A matrix
// run names each engine's share after the wall-clock figure; a plain run has
// no split and keeps the single figure.
func formatEngineSplit(duration time.Duration, engines []engineDuration) string {
	if len(engines) == 0 {
		return fmt.Sprintf("%dms", duration.Milliseconds())
	}
	parts := make([]string, len(engines))
	for i, engine := range engines {
		parts[i] = fmt.Sprintf("%s %dms", engine.Runner, engine.Duration.Milliseconds())
	}
	return fmt.Sprintf("%dms: %s", duration.Milliseconds(), strings.Join(parts, ", "))
}
