   ...
   than one, and the tables as a menu. *}	<aside class="rail">
		...
		<form class="rail__switch" method="post" action="/session/connection">
			...
			<select id="rail-connection" name="connection_id" data-submit>
				...
				<option value="{$option.id}"{if $option.id == $ctx.connection_id} selected{/if}>{$option.name}</option>
				{/foreach}
			</select>
		...
		<form class="rail__switch" method="post" action="/session/schema">
			...
			<select id="rail-schema" name="schema" data-submit>
				...
				<option value="{$name}"{if $name == $ctx.schema_name} selected{/if}>{$name}</option>
				{/foreach}
			</select>
			<noscript><button type="submit">Switch</button></noscript>
		</form>
		...
		{if $sidebar.error}
		<p class="rail__error"><span class="dot dot--bad"></span>{$sidebar.error}</p>
		{/if}

		<ul class="rail__tables" id="rail-tables">
			{foreach $sidebar.tables as $entry}
			<li><a href="/t/{$entry.name|urlencode}"{if $entry.name == $sidebar.active_table} class="active"{/if}>{$entry.name}</a></li>
			{else}
