...
<form class="filter-bar" method="get" action="/admin/audit">
	<div class="field">
		...
		<select id="a-action" name="action">
			...
			<option value="{$action}"{if $action == $filter.action} selected{/if}>{$action}</option>
			{/foreach}
		</select>
	</div>
	<div class="field">
		...
		<select id="a-user" name="user">
			...
			<option value="{$actor.id}"{if $actor.id == $filter.user_id} selected{/if}>{$actor.username}</option>
			{/foreach}
		</select>
	</div>
	<div class="field">
		...
		<select id="a-conn" name="connection">
			...
			<option value="{$actor.id}"{if $actor.id == $filter.connection_id} selected{/if}>{$actor.name}</option>
			{/foreach}
		</select>
	</div>
	<div class="field">
		<label for="a-table">Table</label>
		<input id="a-table" name="table" value="{$filter.rel_table}" autocomplete="off">
	</div>
...
<table>
	...
	<tbody>
		...
		<tr>
			...
			<td><span class="status{if $entry.action == "denied"} s4xx{elseif $entry.action == "drop"} s5xx{elseif $entry.action == "truncate"} s5xx{/if}">{$entry.action}</span></td>
			<td class="name">{if $entry.username}{$entry.username}{elseif $entry.user_id}<span class="dim">#{$entry.user_id}</span>{else}<span class="dim">&mdash;</span>{/if}</td>
			<td class="dim c-medium">{if $entry.connection_name}{$entry.connection_name}{/if}{if $entry.rel_table} / {$entry.rel_table}{/if}</td>
			<td class="flex">{$entry.message}</td>
			<td class="dim c-wide"><code class="truncate">{$entry.payload}</code></td>
		</tr>
		{/foreach}
	</tbody>
</table>
