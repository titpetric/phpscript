...
{if $result}
	...
	{else}
	<p class="notice notice--bad"><span class="dot dot--bad"></span>{$result.message|h}</p>
	{/if}
{/if}

<dl class="detail">
	<div><dt>Driver</dt><dd>{$connection.driver|h}</dd></div>
	<div><dt>DSN</dt><dd><code>{$redacted|h}</code></dd></div>
	<div><dt>Status</dt><dd>{$connection.status|h}{if $connection.status_message} &mdash; {$connection.status_message|h}{/if}</dd></div>
	<div><dt>Last checked</dt><dd>{$connection.checked_at|hv}</dd></div>
	<div><dt>Created</dt><dd>{$connection.created_at|hv}</dd></div>
</dl>
...
<form method="post" action="/admin/connection/{$connection.id}" class="stack">
	...
	<div class="field grow">
		...
		<input id="c-dsn" name="dsn" value="{$connection.dsn|h}" autocomplete="off" required>
		<small>Shown in full here because this is the form that changes it.</small>
	</div>
	<div class="field">
		<label for="c-schema">Default schema</label>
		<input id="c-schema" name="schema" value="{$connection.default_schema|hv}" autocomplete="off">
	</div>
...
<table>
	...
	<tbody>
		...
		<tr>
			<td class="name flex"><a href="/admin/group/{$grant.id}">{$grant.name|h}</a></td>
...
<div class="danger-zone">
	<form method="post" action="/admin/connection/{$connection.id}/delete" class="danger-form">
		...
		<div class="danger-form__confirm">
			<input name="confirmation" placeholder="Type {$connection.name|h} to confirm" autocomplete="off"
				data-confirm-name="{$connection.name|h}">
