...
<form method="post" action="/admin/group/{$group.id}" class="stack">
	...
	<div class="field">
		...
		<input id="g-name" name="name" value="{$group.name|h}" autocomplete="off" required>
	</div>
	<div class="field grow">
		<label for="g-desc">Description</label>
		<input id="g-desc" name="description" value="{$group.description|hv}" autocomplete="off">
	</div>
	<div class="field">
		<label for="g-policy">Destructive actions</label>
		<select id="g-policy" name="policy">
			{foreach $policies as $policy}
			<option value="{$policy|h}"{if $policy == $group.destructive_policy} selected{/if}>{$policy|h}</option>
			{/foreach}
		</select>
	</div>
	...
	<table>
		...
		<tbody>
			...
			<tr>
				...
				<td class="dim c-medium flex">{$entry.driver|h}</td>
			</tr>
			{/foreach}
		</tbody>
	</table>
...
<form method="post" action="/admin/group/{$group.id}" class="stack">
	...
	<label class="check"><input type="checkbox" name="user_{$entry.id}" value="1"{if in_array($entry.id, $member_ids)} checked{/if}>
		{$entry.username|h}{if $entry.is_admin} <span class="status">admin</span>{/if}</label>
	{/foreach}
...
<div class="danger-zone">
	<form method="post" action="/admin/group/{$group.id}/delete" class="danger-form" data-confirm="Delete {$group.name|h}?">
		<input type="hidden" name="csrf_token" value="{$ctx.csrf_token|h}">
		<div>
