...
- **Variable types are immutable.** The first non-null value declares a variable's type; another type later is a `RuntimeException` at run time and a fatal lint finding. The rule and its allowances (number class, `false` sentinel, null-first, `foreach` binding) live in `internal/phpval` and `tests/fixtures/types/` pins them on both engines. [docs/design.md](docs/design.md)
