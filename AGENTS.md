...
- **Variable types are dynamic; one type per name is a lint convention, not a contract.** Runtime enforcement was tried and reversed - it stopped existing code from running. The linter's advisory "no reassignment" finding stays; do not re-add throws. [docs/design.md](docs/design.md)
...
- Fixture paths: `phpscript test ./...` runs a tree; `phpscript test .` matches only fixtures directly in the directory and can silently pass. Every pipeline job spells the `./...`, because without it a job points at a directory of folders and passes over nothing.
- A change to language or runtime behaviour lands with a fixture in the matching `tests/fixtures/<area>/` folder, either a `.phpt` document or a `<name>_test.php` body with its `<name>_test.txt` output beside it. A new area is a new folder; nothing registers it. The fixture's own folder is its include root; copy shared support files rather than climbing out.
...
- Generated files are regenerated, never hand-edited: `docs/reference/README.md` (from `php README.php`), `docs/reference/extensions/implemented-apis.md` (from `scripts/list-apis`), `docs/test-fixtures.md` (from `phpscript test --matrix -o`, deliberately without `--profile`), `docs/coverage/phpscript.md` and `docs/coverage/phpscript-detail.md` (from `atkins cover`, reading the `cover/pkg.cov` the test job wrote), `docs/coverage/dbadmin.md` and `docs/coverage/dbadmin-detail.md` (from `atkins gen:coverage:dbadmin`, reading the `cover/dbadmin.file.txt` the demo venom suite read off the running service). A new doc under `docs/reference/` must be listed in `docs/reference/README.yml` or `README.php` throws.
